...
// A script to help with deploying contracts in a multi-chain environment and save deployment info
// Modified from Synapse's BaseScript.sol
// https://github.com/synapsecns/synapse-contracts/blob/57d257bde381cf5306257b87c3d39b3c4f74098a/script/utils/BaseScript.sol
abstract contract BaseScript is Script {
    ...
    function loadDeployment(string memory chain, string memory contractName) public returns (address deployment) {
    ...
    function saveDeployment(
        string memory chain,
        string memory sourceContractName,
        string memory saveContractName,
        address deployedAt
    ) public {
        ...
        deployment.write(_deploymentPath(chain, saveContractName));
        ...
        inputs[1] = "--argfile";
        inputs[2] = "artifact";
        inputs[3] = _artifactPath(sourceContractName);
        // Set value for ".abi" key to artifact's ABI
        inputs[4] = ".abi = $artifact.abi";
        inputs[5] = _deploymentPath(chain, saveContractName);
        bytes memory full = vm.ffi(inputs);
        // Finally, print the updated deployment JSON
        string(full).write(_deploymentPath(chain, saveContractName));
        console2.log("Deployed: [%s] as [%s] at %s", sourceContractName, saveContractName, deployedAt);
    }
    ...
    function saveDeployConfig(
        ...
        console2.log("Saved: config for [%s] on [%s]", contractName, chain);
    }
